...
public class RocksDBKeyValueService extends RocksDBKeyValueRepository<String, String> {
    ...
    public Optional<String> getProcessInstance(String processInstanceId) throws IOException {
        try {
            ...
            if (values.isPresent()) {
                ...
                Optional<KafkaExternalTask> userTask = kafkaExternalTasks.getKafkaExternalTasks().stream().filter(this::isUserTask).findAny();
    ...
    private boolean isUserTask(KafkaExternalTask item) {
        return USER_TASK.getEvent().equals(item.getType()) && !item.isTaskComplete();
    }
}
